...
class StaffPendingSecurityTest {
    ...
    @Test void pendingDeliveriesRequireCashRoleEvenWhenOtherRolesCanReadGeneralDeliveries() {
        try(var context=new AnnotationConfigApplicationContext(Config.class)) {
            ...
            when(service.findPendingPickups(anyString(), any(), any())).thenReturn(Page.empty());
            for(var role:List.of("SOCIO_COMERCIAL","JEFA_CUENTAS","AUXILIAR_CUENTAS","ADMIN")) {
                ...
                assertThatThrownBy(() -> controller.pending("CONFIRMATION",null,PageRequest.of(0,10))).isInstanceOf(AccessDeniedException.class);
            }
            ...
            assertThat(controller.pending("CONFIRMATION",null,PageRequest.of(0,10)).getStatusCode().is2xxSuccessful()).isTrue();
            verify(service,times(1)).findPendingPickups("CONFIRMATION",null,PageRequest.of(0,10));
        }
    }
}
